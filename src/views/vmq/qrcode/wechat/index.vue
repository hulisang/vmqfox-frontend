<template>
  <div class="qrcode-container">
    ...
    <!-- 添加二维码弹窗 -->
    <el-dialog v-model="showAddDialog" title="添加微信二维码" @close="resetForm">
...
const fetchQrcodeList = async () => {
  loading.value = true
  try {
    const res = await VmqService.getWxQrcodes(searchParams.value)
...
const handleAdd = async () => {
  ...
  await addFormRef.value.validate(async (valid) => {
    if (valid) {
      try {
        await VmqService.addWxQrcode(addForm)
